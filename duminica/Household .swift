...
class Household : UITableViewCell,  UICollectionViewDataSource, UICollectionViewDelegate, CLLocationManagerDelegate {
        ...
        circleQuery?.observeReady({
            self.fetchAllPost {(posts) in
                ...
                self.sweetToDisplay = self.sweets.filter{
                    $0.postCategory.contains("HOUSEHOLD ITEMS")
                }
...
extension Household : UICollectionViewDelegateFlowLayout {
